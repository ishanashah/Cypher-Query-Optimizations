//Query 5 Source-Target Optimized
CALL gds.graph.project.cypher(
  'myGraphRed',
  'MATCH (n:Location) RETURN id(n) AS id',
  'MATCH (n:Location {color: "RED"})-[r:ROAD]-(m:Location {color: "RED"}) RETURN id(n) AS source, id(m) AS target, r.cost AS cost')
YIELD graphName as graphRed, nodeCount AS nodesRed, relationshipCount AS relsRed
CALL gds.graph.project.cypher(
  'myGraphBlue',
  'MATCH (n:Location) RETURN id(n) AS id',
  'MATCH (n:Location {color: "BLUE"})-[r:ROAD]-(m:Location {color: "BLUE"}) RETURN id(n) AS source, id(m) AS target, r.cost AS cost')
